<template>
  <!--
    ChatView — qa-chat 2026-05-08 against docs/design/notion.md.
    Two-column layout: session history sidebar (left) + chat area (right).
  -->
  <div class="h-screen flex flex-col bg-notion-surface-soft text-notion-ink">
    ...
    <div class="bg-notion-brand-navy text-notion-on-dark px-6 py-5 flex-shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-[13px] text-notion-on-dark-muted mt-0.5">基于知识库 + 私有数据的 RAG 问答</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="w-72 flex-shrink-0 bg-notion-canvas border-r border-notion-hairline flex flex-col">
      ...
      <div class="flex-1 flex flex-col bg-notion-surface-soft overflow-hidden">
        ...
        <!-- Toolbar -->
        <div class="bg-notion-canvas border-b border-notion-hairline px-6 py-3 flex items-center justify-between gap-4 flex-shrink-0">
        ...
        <!-- Input area -->
        <div class="bg-notion-canvas border-t border-notion-hairline px-6 py-3 flex-shrink-0">
          <div class="max-w-3xl mx-auto flex items-end gap-2">
            ...
            <button
              ...
              :class="[
                'h-10 px-4 rounded-md text-[14px] font-medium transition-colors',
...
// ===== Save-to-notes =====
// Per-message state; keyed by message index in the current session.
// Each entry: { phase, title, directory, content, submitting, error, savedDirectory }
...
function defaultTitleForMessage(idx) {
  // Prefer the user message immediately preceding the assistant message —
  // that's the question the answer is about.
...
function toggleScope(name) {
  ...
  if (has && scope.value.length === 1) {
    // Refuse to deactivate the last active scope
    return
  }
  scope.value = has
