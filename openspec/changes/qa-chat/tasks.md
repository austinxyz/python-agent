...
- [ ] 6.3 Apply UI design system from `docs/design/notion.md`: brand-navy hero header, sidebar styling per the established notion-* token namespace in `frontend/tailwind.config.cjs`, bubble styles for user/assistant messages (user = primary purple bg, assistant = canvas with hairline border), scope toggle active state via pill-tab pattern
