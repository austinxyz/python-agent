...
- `url`: fetch the URL with `httpx` (15 s timeout, `follow_redirects=True`); SSRF validation MUST be applied to both the initial URL and the final URL after redirects (using `ipaddress` module to block RFC 1918 ranges); extract visible text from HTML using `html.parser`; non-HTML responses store raw text up to 1 MB
...
- **WHEN** source_type is `"url"` and the content is a valid PDF
- **THEN** Fetch Node populates `raw_content` with the concatenated text of all pages

#### Scenario: URL fetch respects timeout
- **WHEN** source_type is `"url"` and the remote server does not respond within 15 seconds
- **THEN** Fetch Node sets `status="error"` with a timeout message

#### Scenario: Post-redirect SSRF is blocked
- **WHEN** source_type is `"url"`, the initial URL is a public hostname, but the server redirects to a private IP (e.g., `http://192.168.1.1/`)
- **THEN** Fetch Node sets `status="error"` with an SSRF error message and does not return the response body
