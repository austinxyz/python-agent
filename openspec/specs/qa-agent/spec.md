...
The QA agent tools (`search_knowledge`, `search_private`, `get_entry`) SHALL be independently callable Python functions that accept a mocked Qdrant client, enabling unit tests without a live Qdrant or LLM. **Default `limit` for both `search_knowledge` and `search_private` SHALL be 10** (raised from 5 on 2026-05-08 by the `chat-retrieval-bump` change) so long documents whose relevant content sits at rank 6-10 are still surfaced.
