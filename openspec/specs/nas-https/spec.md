# nas-https Specification

## Purpose
Brings CA-trusted HTTPS to the NAS-hosted python-agent via Tailscale's auto-Let's-Encrypt mechanism. Previously the app ran on plain HTTP (`http://10.0.0.20:8910`), which forced `SESSION_COOKIE_SECURE=false` and blocked Google Sign-In (GSI requires HTTPS). This capability replaces the plain-HTTP entry point with `https://python-agent.<tailnet-id>.ts.net`, closes both security gaps, and adds an admin cert-status dashboard for monitoring certificate health.
## Requirements
